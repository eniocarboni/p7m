...
T_Test "$?" "find_command() 1/2" "bash - several parameters"
find_command "bash" | grep -q bash
T_Test "$?" "find_command() 2/2" "bash - single parameter"
...
T_Test "$?" "del_oldtmp_file() 1/3" "- 2 file and first in \$file global var"
del_oldtmp_file "$f2_1" "$f2_2"
test -f $f2_1 -a ! -f "$f2_2"
T_Test "$?" "del_oldtmp_file() 2/3" "- 2 file and first in \$p7m_attach global var"
del_oldtmp_file "$f3"
test -f "$f3"
T_Test "$?" "del_oldtmp_file() 3/3" "- template not p7m_"
